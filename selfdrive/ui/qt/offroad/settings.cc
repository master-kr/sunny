...
DevicePanel::DevicePanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  connect(resetCalibBtn, &ButtonControl::clicked, [&]() {
    if (uiState()->engaged()) {
      ConfirmationDialog::alert(tr("Disengage openpilot before resetting calibration."), this);
    } else if (ConfirmationDialog::confirm(tr("Reset calibration and reboot the device?"), tr("Reset"), this)) {
  ...
  QObject::connect(uiState(), &UIState::offroadTransition, [=](bool offroad) {
    for (auto btn : findChildren<ButtonControl *>()) {
      if (btn != errorBtn && btn != resetCalibBtn) {
