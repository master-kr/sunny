...
void AnnotatedCameraWidget::updateTpms(const SubMaster &sm) {
  if (!sm.updated("can")) return;

  for (const auto &frame : sm["can"].getCan()) {
    const auto dat = frame.getDat();
    if (frame.getAddress() != 0x3A0 || dat.size() < 9) continue;

    // Hyundai CAN-FD TPMS (0x3A0): four pressure bytes followed by a
    // two-bit unit multiplier. The resulting values are PSI.
    const int unit = (dat[8] >> 3) & 0x3;
    const float scale = unit > 0 ? unit * 0.725f : 1.0f;
    tpms_pressure = {
      dat[4] * scale,
      dat[5] * scale,
      dat[6] * scale,
    ...
    tpms_last_update = millis_since_boot();
  }
}

void AnnotatedCameraWidget::drawTpms(QPainter &painter, const UIState *s) {
  if (tpms_last_update <= 0.0 || millis_since_boot() - tpms_last_update > 10000.0) return;

  const int icon_offset = UI_BORDER_SIZE + btn_size / 2;
  const int center_x = rightHandDM ? width() - icon_offset : icon_offset;
  const int driver_icon_y = height() - icon_offset - s->scene.rn_offset;
  const int center_y = driver_icon_y - 285;
  const int column_offset = 46;
  const int row_offset = 55;

  painter.save();
  painter.setFont(InterFont(38, QFont::Bold));

  for (int i = 0; i < static_cast<int>(tpms_pressure.size()); ++i) {
    const float pressure = tpms_pressure[i];
    const bool valid = pressure >= 5.0f && pressure <= 60.0f;
    const QString text = valid ? QString::number(std::lround(pressure)) : QString("-");
    painter.setPen(valid && pressure < 31.0f ? redColor() : whiteColor());

    const bool right = i == 1 || i == 3;
    const bool rear = i >= 2;
    const QRect text_rect(center_x + (right ? column_offset : -column_offset) - 42,
                          center_y + (rear ? row_offset : -row_offset) - 27,
                          84, 54);
